...
export const TOOL_DEFINITIONS = {
  ...
  create_contact: {
    ...
    description: 'Create a new CRM contact from information collected during the conversation. ' +
                 'Call this when you know the user\'s name OR phone. ' +
                 'Only call once per conversation. ' +
                 'On WhatsApp the phone is already known — do NOT ask for it. ' +
                 'On Telegram/Facebook the name is already known — do NOT ask for it. ' +
                 'Only ask for additional info (email, company) if it is relevant.',
    input_schema: {
      type:       'object',
      required:   ['firstName'],
      properties: {
        ...
        items: {
          ...
          items: {
            ...
            properties: {
              ...
              imageUrl:    { type: 'string', description: 'Product image URL from lookup_products (pass through as-is)' },
            },
          },
 ...
 * @param {object} context     — { organizationId, conversationId, contactId?, dealId?, appUrl? }
...
async function _createContact(input, { organizationId, conversationId, channel, metadata }) {
  const { firstName, lastName, email, phone, company, notes } = input

  // Parse channel metadata for pre-known info
  const meta = metadata || {}

  // For Telegram: use fromName as name fallback, telegramChatId as identifier
  // For WhatsApp: use whatsappFrom (phone number) as phone fallback
  // For Facebook: use fromName as name fallback
  const resolvedFirstName = firstName || (meta.fromName?.split(' ')[0]) || 'Desconocido'
  const resolvedLastName  = lastName  || (meta.fromName?.split(' ').slice(1).join(' ') || null)

  // WhatsApp from number is already a phone
  let resolvedPhone = phone || null
  if (!resolvedPhone && channel === 'whatsapp' && meta.whatsappFrom) {
    resolvedPhone = meta.whatsappFrom
  }

  // Telegram/Facebook: never ask for phone — channel username as notes fallback
  const resolvedNotes = notes || (meta.fromUsername ? `@${meta.fromUsername}` : null)
    ...
    const existing = await prisma.contact.findFirst({
      where: {
        ...
        OR: [
          ...
          resolvedPhone ? { phone: { contains: resolvedPhone.replace(/\D/g, '').slice(-10) } } : undefined,
        ].filter(Boolean),
      },
...
async function _lookupProducts(input, { organizationId }) {
  const { query } = input
  if (!query?.trim()) {
  ...
  const products = await prisma.product.findMany({
    ...
    select:  { id: true, sku: true, name: true, unit: true, sellingPrice: true, costPrice: true, imageUrl: true },
    orderBy: { name: 'asc' },
    take:    20,
  })
...
async function _createQuote(input, { organizationId, conversationId, contactId, dealId, appUrl }) {
  ...
  const baseUrl   = appUrl || process.env.NEXT_PUBLIC_APP_URL || 'https://crm.hittek.mx'
  const publicUrl = `${baseUrl}/q/${shareToken}`
