...
export async function checkPlanLimit(prisma, organizationId, entity) {
  ...
  const countMap = {
    ...
    knowledgeBases: () => prisma.knowledgeBase.count({ where: { organizationId: organizationId } }),
    kbDocuments:    () => prisma.knowledgeBaseDocument.count({ where: { organizationId: organizationId } }),
  }
