...
export function buildSystemPrompt(chatbot, contextText, { prevSummary, contactContext, enabledTools } = {}) {
  const botName = chatbot?.name || 'Asistente'

  const sections = [
    `Eres ${botName}, un asistente de soporte.`,
  ...
  if (enabledTools?.length) {
    ...
    if (enabledTools.includes('lookup_products') || enabledTools.includes('create_quote')) {
      hints.push('Cuando el usuario pida precios o una cotización: primero llama lookup_products para cada producto mencionado, luego llama create_quote con los precios del catálogo.')
    }
    if (enabledTools.includes('create_contact')) {
      hints.push('Cuando el usuario proporcione su nombre y/o teléfono/email, llama create_contact para registrarlo.')
    }
    if (hints.length) {
...
export async function processMessage({ chatbot, channel, sessionId, userMessage, metadata }) {
  ...
  const toolContext = { orgId, conversationId, contactId: conv.contactId ?? null, appUrl }
  ...
  // Async summary generation (fire-and-forget, no user-facing latency)
  if (autoResolved) {
  ...
  return { reply, conversationId, escalated: false, isNew, handedOff: false, autoResolved }
}
