...
export function buildSystemPrompt(chatbot, contextText, { prevSummary, contactContext } = {}) {
...
async function findContactId(orgId, channel, sessionId) {
  ...
  // sessionId format: "wa-{phoneNumber}" e.g. "wa-521234567890"
  const rawPhone = sessionId.replace(/^wa-/, '')
  if (!rawPhone || rawPhone.length < 7) return null

  // Normalise: strip leading +, spaces, dashes, parentheses
  const digits = rawPhone.replace(/\D/g, '')

  // Try exact match on phone or mobile field, or suffix match (last 10 digits)
...
export async function processMessage({ chatbot, channel, sessionId, userMessage, metadata }) {
  ...
  try {
    ...
    rawReply = response.content?.[0]?.text?.trim() || rawReply
  } catch (err) {
