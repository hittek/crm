...
export async function processMessage({ chatbot, channel, sessionId, userMessage, metadata }) {
  ...
  // Escalation check
  if (chatbot.escalationPhrase && text.toLowerCase().includes(chatbot.escalationPhrase.toLowerCase())) {
    ...
    try {
      ...
      notifications.chatEscalated({ id: conversationId }, chatbot, orgId).catch(() => {})
    } catch (_) {}
