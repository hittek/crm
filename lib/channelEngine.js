...
async function loadPrevSessionContext(chatbotId, sessionId, channel) {
  const prev = await prisma.conversation.findFirst({
    where:   { ...(chatbotId != null && { chatbotId }), sessionId, channel, status: 'resolved' },
 ...
 */
async function findContactId(organizationId, channel, sessionId) {
 ...
 */
async function autoCreateContact(organizationId, channel, sessionId) {
  if (channel !== 'whatsapp') return null
  ...
  try {
    ...
    if (err.code === 'P2002') {
      return findContactId(organizationId, channel, sessionId)
    }
 ...
 */
async function autoCreateDeal(organizationId, contactId, chatbot, channel) {
 ...
 */
export async function logConversationActivity(conversationId, { organizationId, contactId, outcome, channel, chatbotName = null }) {
...
export async function processMessage({ chatbot, channel, sessionId, userMessage, metadata }) {
  const { id: chatbotId, organizationId, kbId } = chatbot
  const text = userMessage.trim()

  // ── Reset command (/new, /start, etc.) ────────────────────────────────────
  if (isResetCommand(text)) {
    await prisma.conversation.updateMany({
      where: { ...(chatbotId != null && { chatbotId }), sessionId, channel, status: { in: ['open', 'escalated'] } },
  ...
  if (!conv) {
    ...
    contactMatch = await findContactId(organizationId, channel, sessionId)

    // --- Auto-create contact if enabled and no match found ---
    if (!contactMatch && chatbot.autoCreateContact) {
      contactMatch = await autoCreateContact(organizationId, channel, sessionId)
    }
    ...
    if (chatbot.autoCreateDeal && contactMatch?.id) {
      autoCreateDeal(organizationId, contactMatch.id, chatbot, channel).catch(err =>
        console.error('[channelEngine] autoCreateDeal failed:', err.message)
      )
    }
  }
  ...
  let chunks = []
  try {
    chunks = await searchChunks({ query: text, kbId, organizationId, limit: 5 })
  } catch (err) {
  ...
  const toolContext = { organizationId, conversationId, contactId: conv.contactId ?? null, appUrl, imageAttachments: [], channel, metadata }
      ...
      for (const block of toolUseBlocks) {
        ...
        if (block.name === 'create_contact' && result.ok && result.data?.contactId) {
          ...
          if (chatbot.autoCreateDeal && !result.data.existing) {
            autoCreateDeal(organizationId, newContactId, chatbot, channel)
