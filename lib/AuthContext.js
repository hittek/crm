...
// Route patterns that are always public (checked as prefix against router.pathname)
const PUBLIC_PREFIXES = ['/q/']

function isPublicRoute(pathname) {
  return PUBLIC_PAGES.includes(pathname) || PUBLIC_PREFIXES.some(p => pathname.startsWith(p))
}
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    if (!isLoading && !user && !isPublicRoute(router.pathname)) {
  ...
  const login = useCallback(async (email, password) => {
    ...
    try {
      ...
      setUser(data.user)
      // Re-fetch me to populate org billing state, then reload org settings
      await checkAuth()
